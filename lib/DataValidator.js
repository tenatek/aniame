...
class DataValidator {
   ...
   * @param {JSONPath[]} validationResult The validation result.
   * @return {Promise<void>}
   */
  static async checkNode(
    ...
    if (descriptor.type === 'array') {
      if (!(dataNode instanceof Array)) {
        ...
        validationResult.push(dataPath);
      } else {
        ...
        if (enforceRequired) {
          ...
          for (let key in descriptor.properties) {
            if (
              ...
              validationResult.push(propertyDataPath);
            }
          }
        }
   ...
   * @param {function} [refCallback] A callback for custom validation on references.
   * @return {Promise<JSONPath[]>} The validation result.
   */
